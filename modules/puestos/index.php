...
<style>
    ...
    #tabla_puestos_filter {
        margin-bottom: 15px;
    }

    #tabla_puestos_length {
        margin-bottom: 15px;
    }
...
<div class="container mt-4">
    <h2 class="text-center">Puestos de Trabajo</h2>
    <div class="d-flex justify-content-between align-items-center">
        <button id="btnAgregar" class="btn btn-success" data-toggle="modal" data-target="#modalPuesto">
            Agregar Puestos
        </button>
    ...
    <div class="table-responsive mt-4">
        <table class="table table-hover" id="tabla_puestos">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="submit" class="btn btn-primary" form="formularioPuesto">Guardar</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        $('#btnAgregar').click(function() {
            limpiarFormulario();
            $('#modalPuesto').modal('show'); // Asegúrate de que el ID coincida con el de tu modal
        });

        // Función para limpiar el formulario (útil para reutilizar el modal)
        function limpiarFormulario() {
            $('#formularioPuesto')[0].reset();
            // Puedes agregar más lógica aquí si es necesario
        }

        // Evento para el envío del formulario
        $('#formularioPuesto').submit(function(e) {
            e.preventDefault();
            var datosFormulario = $(this).serialize(); // Captura los datos del formulario

            $.ajax({
                url: '/rrhh-dismafer/puestos/ajax',
                type: 'POST',
                data: datosFormulario,
                success: function(response) {
                    $('#modalPuesto').modal('hide'); // Cerrar el modal después de la respuesta exitosa
